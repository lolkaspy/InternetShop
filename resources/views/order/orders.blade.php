...
<div class="container">
    ...
    ">
        ...
        <div class="col-sm-2">
            ...
            <input id="low_total" name="low_total" type="number" min="0" class="form-control" value="{{request()->low_total}}"  placeholder="{{round($minTotal)}}"/>
        </div>
        <div class="col-sm-2">
            <label for="high_total" class="label-14pt">до</label>
            <input id="high_total" name="high_total" type="number" min="0" class="form-control" value="{{request()->high_total}}"  placeholder="{{round($maxTotal)}}"/>
        </div>
        <div class="col-sm-2">
            <label for="created_at" class="label-14pt">Дата</label>
            <input id="created_at" name="created_at" type="date"  class="form-control" value="{{request()->created_at}}"/>
        </div>
        @if(Auth::check() && Auth::user()->role_id == 1)
            <div class="col-sm-3">
        ...
        <div class="col-sm-2">
            ...
            <select name="sort_by" id="sort_by" class="form-control form-select" >
                ...
                @if(Auth::check() && Auth::user()->role_id == 1)
                <option value="user" {{ (request()->sort_by == 'user') ? 'selected' : '' }}>Пользователь</option>
                @endif
            </select>
        </div>
        <div class="col-sm-2">
            ...
            <div class="col-sm-2">
                ...
                @endif
                    @if(Auth::check() && Auth::user()->role_id == 2)
                        <a href="{{route('orders.index')}}" class="btn btn-primary bg-black-pastel text-white form-control ">
                            {{ __('Сбросить фильтры') }}
                        </a>
                    @endif
            </div>
            ...
            @endswitch
                ...
                <form action="{{ route('order.updateState', $order->id) }}" method="POST">
                    ...
                    <select name="state_change" id="state_change" class="form-control form-select w-50">
                        <option value="-1" {{ $order->state == -1 ? 'selected' : '' }}>Отменённый</option>
                        <option value="0" {{ $order->state == 0 ? 'selected' : '' }}>Новый</option>
                        <option value="1" {{ $order->state == 1 ? 'selected' : '' }}>Подтверждённый</option>
                    </select>
                    <button type="submit" class="btn btn-primary bg-black-pastel text-white">Обновить статус</button>
                    </div>
                </form>
                    @endif
            </td>
            ...
            @if(Auth::check() && Auth::user()->role_id == 2)
                <td>
                    <form action="{{ route('order.cancel', $order->id) }}" method="POST">
                        @csrf
                        <button type="submit" class="btn btn-primary bg-black-pastel text-white">Отменить заказ</button>
                    </form>
                </td>
            @endif
        </tr>
