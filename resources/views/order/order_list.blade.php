...
@section('content')
    ...
    <div class="container">
        ...
        <hr>
        <form method="GET" class="row g-3 d-flex justify-content-center" action="{{route('order_list.show', [$orderList->first()->order_id])}}">
           <div class="col-sm-3">
            ...
            <div class="col-sm-2">
                ...
                <input id="low_subtotal" name="low_subtotal" type="number" min="0" class="form-control" value="{{request()->low_subtotal}}"  placeholder="{{round($minSubtotal)}}"/>
            </div>
            <div class="col-sm-2">
                <label for="high_subtotal" class="label-14pt">до</label>
                <input id="high_subtotal" name="high_subtotal" type="number" min="0" class="form-control" value="{{request()->high_subtotal}}"  placeholder="{{round($maxSubtotal)}}"/>
            </div>
            <div class="col-sm-2">
                <label for="sort_by" class="label-14pt">Сортировка</label>
                <select name="sort_by" id="sort_by" class="form-control form-select" >
                    <option value="product_id" {{ (request()->sort_by == 'product_id') ? 'selected' : '' }}>№ товара</option>
            ...
            <div class="row g-3 d-flex justify-content-center">
                <div class="col-sm-2">
                    <a href="{{route('order_list.show', [$orderList->first()->order_id])}}" class="btn btn-primary bg-black-pastel text-white form-control ">
                        {{ __('Сбросить фильтры') }}
                    </a>
    ...
    <tr>
        ...
        <td><a href="{{route('product.show', [$order->product->slug])}}"><button class="btn btn-primary bg-black-pastel text-white">Посмотреть информацию</button></a></td>
