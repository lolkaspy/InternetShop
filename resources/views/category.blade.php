...
@section('content')
    <hr>
    <div class="container">
        <div><h1>{{$currentCategory->name}} ({{$products->total()}})</h1></div>
        <hr>
        <form method="GET" class="row g-3 d-flex justify-content-center" action="">
            ...
            <div class="row g-3 d-flex justify-content-center">
                <div class="col-sm-2">
                    <a href="{{route('category.index', ['category' => $currentCategory->slug])}}"
                        ...
                        <td>
                            @if($product->available_quantity > 0)
                                <form action="{{ route('product.addToCart') }}" method="POST">
                                    ...
                                    <button type="submit" class="btn btn-primary bg-black-pastel text-white">Добавить в
                                        корзину
                                    </button>
                                </form>
                            @else
